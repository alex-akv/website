...
cloudShell:
    enabled: true
    folder: site/content/docs/tutorials/frameworks-and-pipelines/metaflow
    editorFile: index.md
---
...
### Install and configure Metaflow client library
...
4. Create a persistent volume claim for the transformers library cache because the cache can grow more than 10GB and GKE's ephemeral storage can provide only up to 10GB on Autopilot mode:

   ```bash
   kubectl apply -f ../metaflow/manifests/cache-pvc.yaml
   ```

5. Create argo-workflows template from the metaflow fine-tuning script:
...
6. Open new terminal and forward port for argo-workflows server. Keep this terminal running to ensure uninterrupted access to the argo-workflows UI.

   ```bash
   kubectl -n argo port-forward svc/argo-server 2746:2746
   ```

7. Open argo-workflows UI at [https://localhost:2746](https://localhost:2746) and go to the [workflow templates](https://localhost:2746/workflow-templates) section. There must be a template that you have created earlier.

   ![argo-workflows-templates](img/argo-workflow-create.png)

8. Select this template and click submit to start the workflow:
...
9. Go to your [Hugging Face](https://huggingface.co/) profile to verify that the fine-tuned model has been uploaded:
